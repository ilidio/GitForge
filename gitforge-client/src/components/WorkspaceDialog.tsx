...
/**
 * Inner content component that owns all local state.
 * It is rendered with a `key` that changes when the dialog opens, so React
 * remounts it on each open — resetting state from props without any effect.
 */
function WorkspaceDialogContent({ workspaces, onOpenChange, onSave }: Omit<WorkspaceDialogProps, 'open'>) {
    // Initialising from props is intentional: this component is always
    // remounted when the dialog opens (via the `key` prop on the wrapper),
    // so there is no stale-state risk and no effect is needed.
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl h-[500px] flex flex-col">
                ...
                {/* key={String(open)} remounts WorkspaceDialogContent each time the dialog
                    opens, which resets all local state from props without needing an effect. */}
